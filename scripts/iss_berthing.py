...
def _av_targets(dc, art_h, targets):
    """dc 로 조인트 위치 타겟 적용 — PhysX 드라이브 직접(= ArticulationView 와 동일 거동, 팔이 타겟에 강하게 버팀)."""
    dc.wake_up_articulation(art_h)
    dc.set_articulation_dof_position_targets(art_h, np.asarray(targets, dtype=np.float32))


def _av_set_positions(dc, art_h, positions):
    """초기 자세 설정 — dc 의 직접 teleport API(set_articulation_dof_positions)는 없으므로
    타겟 구동으로 대체. 강성이 높아 곧 해당 자세로 수렴한다."""
    dc.wake_up_articulation(art_h)
    dc.set_articulation_dof_position_targets(art_h, np.asarray(positions, dtype=np.float32))


def _av_set_gains(dc, art_h, kps, kds):
    """PhysX 드라이브 강성(stiffness)/감쇠(damping) 직접 설정."""
    try:
        props = dc.get_articulation_dof_properties(art_h)
        props["stiffness"] = np.asarray(kps, dtype=np.float32)
        props["damping"] = np.asarray(kds, dtype=np.float32)
        dc.set_articulation_dof_properties(art_h, props)
    except Exception as _e:
        print(f"[gains] dc set_gains 실패(무시): {_e}")
...
def _berthing_generator(stage, world, dc, timeline, simulation_app, station_path=None, spawn_robot=True, robot_prim_path="/World/Robot", chaser_base_path=None, passed_robot_art=None):
    ...
    try:
        while True:
            ...
            # [통합] dc(dynamic_control)로 PhysX 드라이브 직접 제어 (robot_art.apply_action 대신).
            #  - ArticulationView 와 동일 거동이라 팔이 IK 타겟에 강하게 버팀(쫙 펴짐)
            #  - dc 핸들은 매 프레임 재획득 → ADR→berthing 전환 후에도 항상 유효
            _art_h = dc.get_articulation(f"{robot_prim_path}/base_link")
            if _art_h == _dynamic_control.INVALID_HANDLE:
                _art_h = dc.get_articulation(robot_prim_path)
            if _art_h == _dynamic_control.INVALID_HANDLE:
                continue  # 아직 articulation 준비 안 됨 → 다음 프레임
            _ndof = dc.get_articulation_dof_count(_art_h)

            if not gains_set and _ndof > 0:
                _av_set_gains(dc, _art_h, np.ones(_ndof)*10000.0, np.ones(_ndof)*1000.0)
                actual_init_joints = np.zeros(_ndof)
                actual_init_joints[:min(8, _ndof)] = init_joints[:min(8, _ndof)]
                _av_set_positions(dc, _art_h, actual_init_joints)
                # [통합] _final 로봇은 12 DOF(6축+RG2 6관절)
                dof_targets = np.zeros(_ndof)
                gains_set = True
                print(f"[Setup] dc 저수준 제어 초기화 완료 (num_dof={_ndof}).")
            ...
            if PHASE == -1:
                ...
                _av_targets(dc, _art_h, dof_targets)
                ...
                if vision_lost_frames > 15 and approach_dist > 1.0:
                    ...
                    if compliance_active:
                        stiff_kps = np.ones(_ndof) * 10000.0
                        stiff_kds = np.ones(_ndof) * 1000.0
                        _av_set_gains(dc, _art_h, stiff_kps, stiff_kds)
                        compliance_active = False
                ...
                if green_depth is not None and green_depth <= 0.30 and not compliance_active:
                    compliance_kps = np.ones(_ndof) * 10000.0
                    compliance_kds = np.ones(_ndof) * 1000.0
                    for idx in range(6):
                        compliance_kps[idx] = 200.0  # 강성 대폭 인하 (원활한 정렬과 맞물림 유도)
                        compliance_kds[idx] = 20.0   # 댐핑 인하
                    _av_set_gains(dc, _art_h, compliance_kps, compliance_kds)
